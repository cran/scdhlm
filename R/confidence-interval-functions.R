...
#' 
#' @description Calculates an approximate confidence interval given a \code{g_REML} or \code{g_HPS}
#' object, based on a non-central t approximation.
#' 
#' @param g an estimated effect size object of class \code{g_REML} or \code{g_HPS}
...
CI_g.g_REML <- function(g, cover = 0.95, bound = 35) {
  delta <- g$delta_AB
  kappa <- g$kappa
  nu <- g$nu
  V_delta <- kappa^2 * nu / (nu - 2) + delta^2 * (nu / (nu - 2) - 1 / J(nu)^2)
...
CI_g.g_HPS <- function(g, cover = 0.95, bound = 35) {
